...
var component = Vue.component('fwAngleInput', {
    ...
    methods: {
        ...
        click: function click(event) {
            ...
            var newValue = Math.atan2(-targetPoint.y, targetPoint.x);
...
},{"./show-table":17,"./start-editor":18}],12:[function(require,module,exports){
"use strict";
...
var component = Vue.component('fwComplexNumberInput', {
    props: ['value'],
    ...
    methods: {
        ...
        updateMagnitude: function updateMagnitude(magnitude) {
            this.update(magnitude, this.angle);
        },
        update: function update(magnitude, angle) {
            var newValue = Complex.fromPolar(magnitude, angle);
...
},{"complex":7}],14:[function(require,module,exports){
'use strict';

var CompositeSinusoid = require('./composite-sinusoid');
var Color = require('color');

var template = '\n<div class="composite-sinusoid">\n    <canvas ref="canvas"></canvas>\n</div>\n';

var DEFAULT_RESOLUTION = 63;
var DEFAULT_CANVAS_SIZE = 256;
var DEFAULT_ZOOM_FACTOR = 0.6;

var component = Vue.component('fwCompositeSinusoid', {
    ...
    watch: {
        terms: function terms(_terms) {
            this.receiveTerms(_terms);
        }
    },

    methods: {

        receiveTerms: function receiveTerms(terms) {
        ...
        initializeCanvas: function initializeCanvas(size) {
            var canvas = this.$refs.canvas;
            canvas.width = size;
            canvas.height = size;
            var context = canvas.getContext('2d');
        ...
        drawCompositeSinusoid: function drawCompositeSinusoid(terms, resolution) {
            var _this = this;

            var sinusoid = new CompositeSinusoid();
            sinusoid.resolution = resolution;
            sinusoid.terms = terms;
            var samples = sinusoid.samples();
            var points = samples.map(function (sample) {
                return _this.convert(sample);
            });
            this.drawPath(points, resolution);
        },

        drawPath: function drawPath(points, resolution) {
            var hue = 0;
            var hueDelta = 360 / resolution;
            var color = this.createColor(hue);
            var canvas = this.$refs.canvas;
            var context = canvas.getContext('2d');
        ...
        convert: function convert(z) {
            // complex number (0, 0) corresponds to center of canvas
            // center of canvas corresponds to complex number (0, 0)
            var zoomFactor = this.zoomFactor || DEFAULT_ZOOM_FACTOR;
            var canvas = this.$refs.canvas;
            var canvasRadius = Math.min(canvas.width, canvas.height) / 2;
            var scale = canvasRadius * zoomFactor;
            var x = scale * z.real + canvasRadius;
            var y = -scale * z.im + canvasRadius;
            return { x: x, y: y };
        },

        createColor: function createColor(hue) {
            var color = Color({ h: hue, s: 100, l: 50 });
...
},{"./composite-sinusoid":15,"color":6}],15:[function(require,module,exports){
'use strict';
...
var Sinusoid = function () {
    ...
    _createClass(Sinusoid, [{
        key: 'samples',
        value: function samples() {
            var list = [];
            var resolution = Math.max(4, Math.round(this.resolution));
            for (var increment = 0; increment < resolution; increment++) {
                var angle = TAU * increment / resolution;
                var power = new Complex(0, angle).multiply(this.frequency);
                var point = power.exp().multiply(this.coefficient);
                list.push(point);
            }
            return list;
        }
...
var CompositeSinusoid = function () {
    function CompositeSinusoid() {
        _classCallCheck(this, CompositeSinusoid);
    ...
    _createClass(CompositeSinusoid, [{
        key: 'samples',
        value: function samples() {
            var sinusoid = new Sinusoid();
            sinusoid.resolution = this.resolution;
            var list = [];
            for (var index = 0; index < this.resolution; index++) {
                list.push(new Complex(0, 0));
            }
            for (var _index in this.terms) {
                sinusoid.frequency = this.terms[_index].frequency;
                sinusoid.coefficient = this.terms[_index].coefficient;
                var samples = sinusoid.samples();
    ...
    return CompositeSinusoid;
}();

module.exports = CompositeSinusoid;

},{"complex":7}],16:[function(require,module,exports){
'use strict';
...
var component = Vue.component('fwNumberInput', {
    props: ['value'],
    template: template,
    data: function data() {
        return {
    ...
    watch: {
        numberString: function numberString(_numberString) {
            var value = parseFloat(_numberString);
            if (differ(value, this.value)) {
                this.$emit('input', value);
            }
        },
        value: function value(_value, oldValue) {
            if (differ(_value, oldValue)) {
                this.numberString = String(_value);
            }
        }
    }
});
...
var BALANCE = 1;
var CROSS_FREQUENCY = 0;
var MIN_RESOLUTION = 48;
var NUM_ROWS = 10;
var ROW_START = 1;
var ROW_STEP = 1;
var NUM_COLS = 10;
...
function showTable(selector) {
    ...
    return new Vue({
        ...
        watch: {
            ...
            crossFrequency: function crossFrequency() {
                this.updateTable();
            },
            rowStart: function rowStart() {
                this.updateTable();
            },
            rowStep: function rowStep() {
                this.updateTable();
            },
            colStart: function colStart() {
                this.updateTable();
            },
            colStep: function colStep() {
                this.updateTable();
            }
        ...
        methods: {
            ...
            createTable: function createTable() {
                var rowStart = parseFloat(this.rowStart);
                var rowStep = parseFloat(this.rowStep);
                var table = [];
                for (var i = 0; i < this.numRows; i++) {
                    var rowIndex = rowStart + i * rowStep;
            ...
            createRow: function createRow(rowIndex) {
                var colStart = parseFloat(this.colStart);
                var colStep = parseFloat(this.colStep);
                var row = [];
                for (var i = 0; i < this.numRows; i++) {
                    var colIndex = colStart + i * colStep;
                    var cell = this.createTerms(rowIndex, colIndex);
                    row.push(cell);
                }
                return row;
            },

            createTerms: function createTerms(f1, f2) {
                var terms = [];
                var m1 = parseFloat(this.balance);
                var m2 = 1 / m1;
                terms.push(this.createTerm(f1, m1, 0));
                terms.push(this.createTerm(f2, m2, 0));
                var extra = parseInt(this.crossFrequency);
                if (extra) {
            ...
            createTerm: function createTerm(f, mag, angle) {
                return {
                    frequency: f,
            ...
            maxFrequency: function maxFrequency(cell) {
                var frequencies = cell.map(function (term) {
                    return term.frequency;
                });
                return Math.max.apply(Math, frequencies);
            },

            zoomFactor: function zoomFactor(cell) {
                var magnitudes = cell.map(function (term) {
                    return term.coefficient.magnitude();
                });
                var sum = 0;
...
},{"./array-util":12,"complex":7}],18:[function(require,module,exports){
'use strict';
...
},{"./angle-input.component":10,"./complex-number-input.component":13,"./composite-sinusoid.component":14,"./number-input.component":16,"./term-list-editor.component":19,"complex":7}],19:[function(require,module,exports){
'use strict';
...
var component = Vue.component('fwTermListEditor', {
    ...
    computed: {
        ...
        canAdd: function canAdd() {
            ...
            var exists = Boolean(frequency);
            var found = this.terms.findIndex(function (term) {
                return term.frequency === frequency;
            }) >= 0;
            return exists && !found;
        },
        ...
        selectedTerm: function selectedTerm() {
            ...
            return this.terms.find(function (term) {
                return term.frequency === _this.selectedFrequency;
            });
        },

        selectedCoefficient: function selectedCoefficient() {
            return this.selectedTerm ? this.selectedTerm.coefficient : null;
        },

        hasSelectedTerm: function hasSelectedTerm() {
    ...
    methods: {
        ...
        remove: function remove() {
            var i = this.terms.indexOf(this.selectedTerm);
            if (i >= 0) {
        ...
        updateCoefficient: function updateCoefficient(coefficient) {
...
},{"complex":7}],20:[function(require,module,exports){
'use strict';
